...
void AutoPowderGrinder::BlockManager::queueBlocks()
{
	...
	playerPosition.truncate();
	positionsToSearch.push_back(playerPosition);

	Vector3 nextInFront = positionsToSearch.front() + this->d[(int)facing];
	while 
	(
		this->positionMeetsCriteria(nextInFront, facing, playerPosition) && 
		(toBlock(nextInFront).toBreak() || toBlock(nextInFront).isAir())
	)
	{
		positionsToSearch.push_back(nextInFront);
		nextInFront = positionsToSearch.back() + this->d[(int)facing];
	}
	...
	while (!positionsToSearch.empty() && this->blockQueue.size() < AutoPowderGrinder::BlockManager::MAX_QUEUE_SIZE)
	{
		...
		{
			Vector3 neighbouringBlockPos = currentPos + direction;
...
{
	Vector3 localPosition = this->minecraft->player->getPosition();
